...
import com.zygne.stockanalyzer.domain.utils.ColorHelper;
...
public class Main extends Application implements MainPresenter.View,
    ...
    private TextField tfZonesPriceFilter;
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        tfZonesPriceFilter = (TextField) resourceLoader.findView("tf_price_filter_zones");

        taScript = (TextArea) resourceLoader.findView("ta_script");
        taStatus = (TextArea) resourceLoader.findView("ta_status");
        taStatus.textProperty().addListener((ChangeListener<Object>) (observable, oldValue, newValue) -> {
        ...
        labelApiKey = (Label) resourceLoader.findView("label_api_key");
        ...
        cbDataSize = (ChoiceBox) resourceLoader.findView("cb_data_size");

        sliderMinWick = (Slider) resourceLoader.findView("slider_min_wick");
        labelWickSize = (Label) resourceLoader.findView("label_wick_size");

        sliderMinWick.valueProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observableValue, Number oldValue, Number newValue) {
                String text = newValue.intValue() + "%";
                labelWickSize.setText(text);
            }
        });
    ...
    private void createReport() {
        ...
        int dataSize = (Integer) cbDataSize.getSelectionModel().getSelectedItem();

        updateStatusText(null);
        mainPresenter.getZones(ticker, percentile, timeFrame, dataSize, rbFundamentals.isSelected());
    }
    ...
    private void createPowerZones() {
        ...
        String price = tfZonesPriceFilter.getText();

        double priceFilter = -1;

        if (StringUtils.idDouble(price)) {
            priceFilter = Double.parseDouble(price);
        }

        double minSize = sliderMinWick.getValue();

        if (!ticker.isEmpty()) {
            TimeFrame timeFrame = ((TimeFrame) (cbPowerZoneTimeFrame.getSelectionModel().getSelectedItem()));
            liquiditySidePresenter.getSides(ticker, timeFrame, minSize, percentile, priceFilter);
        }
    }
    ...
    public void onComplete(String symbol, String timeFrame) {
        labelTicker.setText("Symbol : " + symbol.toUpperCase() + " - " + timeFrame);
    ...
    public void onTimeFramesPrepared(List<TimeFrame> timeFrames, int defaultSelection) {
        ...
        if (!reportTimeFrame.isEmpty()) {
            ...
            cbPowerZoneTimeFrame.setItems(reportTimeFrame);
            cbPowerZoneTimeFrame.setValue(reportTimeFrame.get(defaultSelection));
        }
    }

    @Override
    public void onDataSizePrepared(List<Integer> data, int defaultSelection) {
        ObservableList<Integer> reportDataSize = FXCollections.observableArrayList();

        reportDataSize.addAll(data);

        if (!reportDataSize.isEmpty()) {
            cbDataSize.setItems(reportDataSize);
            cbDataSize.setValue(reportDataSize.get(defaultSelection));
        }
    }
    ...
    public void onSettingsLoaded(Settings settings) {
        labelApiKey.setText(settings.getApiKey());
