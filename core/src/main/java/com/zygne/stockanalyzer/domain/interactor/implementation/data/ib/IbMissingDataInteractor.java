...
public class IbMissingDataInteractor extends BaseInteractor implements MissingDataInteractor {

    private final Callback callback;
    private final List<BarData> entries;

    public IbMissingDataInteractor(Executor executor, MainThread mainThread, Callback callback, List<BarData> entries) {
    ...
    public void run() {
        ...
        String latestEntry = entries.get(entries.size()-1).getTime();

        long latestTimestamp = System.currentTimeMillis();

        try {
            String time = latestEntry.split(",", -1)[0];
            latestTimestamp = TimeHelper.getTimeStamp("yyyyMMdd HH:mm:ss", time);
