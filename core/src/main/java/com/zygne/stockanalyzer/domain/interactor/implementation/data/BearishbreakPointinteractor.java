...
public class BearishbreakPointinteractor extends BaseInteractor implements BreakPointInteractor {

    private final Callback callback;
    private final List<Histogram> histogramList;
    private final List<LiquidityLevel> liquidityLevelList;

    public BearishbreakPointinteractor(Executor executor, MainThread mainThread, Callback callback, List<Histogram> histogramList, List<LiquidityLevel> liquidityLevelList) {
    ...
    public void run() {
        ...
        for(LiquidityLevel e : liquidityLevelList){
            ...
            for(Histogram h : histogramList) {
                if(h.getDirection() == Histogram.Direction.Down){
                    if(h.inBody(e.price)){
